...
async function generateAuthToken(pwd, secret) {
    const timestamp = Date.now();
    const hash = await sha224(`${pwd}:${timestamp}:${secret}`);
    return btoa(JSON.stringify({ t: timestamp, h: hash.substring(0, 32) }));
}

async function verifyAuthToken(token, pwd, secret, maxAgeMs = 24*60*60*1000) {
    if (!token) return false;
    try {
        const { t, h } = JSON.parse(atob(token));
        if (Date.now() - t > maxAgeMs) return false;
        const expectedHash = (await sha224(`${pwd}:${t}:${secret}`)).substring(0, 32);
...
async function getHomePage(request) {
	...
	const showError = urlObj.searchParams.get('error') === '1';
	return getLoginPage(url, baseUrl, showError);
}
...
<body>
    <div class="card">
        ...
        <p class="footer">Secured by <a href="https://workers.cloudflare.com" target="_blank">Cloudflare Workers</a></p>
    </div>
    <script>
...
<body>
    ...
    <div class="container">
        ...
        <h1 class="title">Proxy Service</h1>
        <p class="subtitle">High-performance proxy service powered by Cloudflare Workers (VLESS + Trojan)</p>
        ...
        <div class="button-group">
            <button onclick="copySingboxSubscription()" class="btn btn-secondary">Copy Sing-box</button>
            <button onclick="copyClashSubscription()" class="btn btn-secondary">Copy Clash</button>
            <button onclick="copySubscription()" class="btn btn-secondary">Copy Universal</button>
        </div>
